  ...
  <div className="body">
   ...
   <main className="mapFrame"><div className="mapViewport" ref={mapRef} onWheel={onWheel} onMouseDown={onDown} onMouseMove={onMove} onMouseUp={onUp} onMouseLeave={onUp}>
    <div className="mapCanvas" style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
      <div className="grid"/><div className="coast uk">UNITED<br/>KINGDOM</div><div className="coast nor">NORWAY</div><div className="coast dk">DENMARK</div><div className="coast nl">NETHERLANDS</div><div className="coast fr">FRANCE</div><div className="sea">NORTH SEA</div>
...
function Contact({p,friendly=false,selected,onClick}){return <button className={'contact '+(friendly?'friendly ':'')+(p.cls==='HOSTILE'?'hostile ':'')+(selected?'selected':'')} style={{left:p.x+'%',top:p.y+'%'}} onClick={e=>{e.stopPropagation();onClick()}}><span className="bug" style={{transform:`rotate(${p.h}deg)`}}/><span className="sym">{friendly?'✈':p.cls==='HOSTILE'?'◆':'✦'}</span><span className="lbl">{p.name}<br/>FL{p.alt} {p.s}KT<br/>HDG {String(p.h).padStart(3,'0')}°</span></button>}
